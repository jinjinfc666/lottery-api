...
//		return list1;
	...
	public Map<String, Object> querySum(String startTime, String endTime, String userName) {
		...
		String sql="SELECT sum(deposit) as sumDeposit, sum(withdrawal) as sumWithdrawal, sum(deduction) as sumDeduction, sum(consumption) as sumConsumption, sum(cancel_amount) as sumCancelAmount, sum(return_prize) as sumReturnPrize, sum(rebate) as sumRebate, sum(main_wallet) as sumMainWallet,sum(red_wallet) as sumRedWallet,sum(profit) as sumProfit FROM member_pl_report "+timeSql+userNameSql;
		logger.debug(sql+"-----------------------------queryLoyTst----SQL--------------------------------");
...
//		return list1;
	...
	public Map<String, Object> queryTeamSum(String startTime, String endTime, String userName,List<?> userNameList) {
		...
		String sql=null;
		if(!StringUtils.isBlank(userName)) {
			sql="SELECT sum(deposit) as sumDeposit, sum(withdrawal) as sumWithdrawal, sum(deduction) as sumDeduction, sum(consumption) as sumConsumption, sum(cancel_amount) as sumCancelAmount, sum(return_prize) as sumReturnPrize, sum(rebate) as sumRebate, sum(main_wallet) as sumMainWallet,sum(red_wallet) as sumRedWallet,sum(recharge_member) as sumRechargeMember,sum(new_members) as sumNewMembers,sum(profit) as sumProfit FROM member_pl_report where "+timeSql+userNameSql;
		}else {
			sql="SELECT sum(deposit) as sumDeposit, sum(withdrawal) as sumWithdrawal, sum(deduction) as sumDeduction, sum(consumption) as sumConsumption, sum(cancel_amount) as sumCancelAmount, sum(return_prize) as sumReturnPrize, sum(rebate) as sumRebate, sum(main_wallet) as sumMainWallet,sum(red_wallet) as sumRedWallet,sum(recharge_member) as sumRechargeMember,sum(new_members) as sumNewMembers,sum(profit) as sumProfit FROM member_pl_report where "+timeSql+userNameSql;
		}
		logger.debug(sql+"-----------------------------queryLoyTst----SQL--------------------------------");
