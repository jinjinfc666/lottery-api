...
public class EleIn5Rx2PlayTypeFacadeImpl  extends DefaultPlayTypeFacadeImpl {
	
	private Logger logger = Logger.getLogger(EleIn5Rx2PlayTypeFacadeImpl.class);
	
	protected String playTypeDesc = "rxfs|任选复式/rxeze|任选二中二/fs-ds";
	...
	public Map<String, Object> preProcessNumber(Map<String, Object> params, UserInfo user) {
		...
		for(String subBetNum : betNumSet) {
			...
			betTotal *= MathUtil.combination(2, len);
		}
	...
	public BigDecimal calPrize(Issue issue, OrderInfo order, UserInfo user) {
		...
		if(winningBetAmount < 2) {
			winningBetAmount = 0;
		}else {
			winningBetAmount = ((Long)MathUtil.combination(2, winningBetAmount)).intValue();
		}
