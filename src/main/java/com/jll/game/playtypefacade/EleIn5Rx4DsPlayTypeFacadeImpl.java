...
public class EleIn5Rx4DsPlayTypeFacadeImpl  extends DefaultPlayTypeFacadeImpl {
	
	private Logger logger = Logger.getLogger(EleIn5Rx4DsPlayTypeFacadeImpl.class);
	
	protected String playTypeDesc = "rx|任选/rxszs|任选四中四/ds";
	...
	public Map<String, Object> calPrize(Issue issue, OrderInfo order, UserInfo user) {
		...
		for(String temp : betNumMul) {
			...
			if(matchCount >= 4) {
				totalWinningBetAmount += ((Long)MathUtil.combination(4, matchCount)).intValue();
			}
		}
