...
public class EleIn5Rx5DsPlayTypeFacadeImpl  extends DefaultPlayTypeFacadeImpl {
	
	private Logger logger = Logger.getLogger(EleIn5Rx5DsPlayTypeFacadeImpl.class);
	
	protected String playTypeDesc = "rx|任选/rxwzw|任选五中五/ds";
	...
	public Map<String, Object> calPrize(Issue issue, OrderInfo order, UserInfo user) {
		...
		for(String temp : betNumMul) {
			...
			if(matchCount >= 5) {
				totalWinningBetAmount += ((Long)MathUtil.combination(5, matchCount)).intValue();
			}
		}
