...
public class Wxh2ZuxPlayTypeFacadeImplTest extends ServiceJunitBase{
	...
	public void testParseBetNumber(){
		...
		Assert.assertTrue(ret.size() == 4000);
		...
		Assert.assertTrue(ret.size() == 42000);
	}
	
	public void testIsMatchWinningNum_winning(){
		String betNum = "12";
		Issue issue = new Issue();
		issue.setRetNum("3,9,6,1,2");
	...
	public void testPreProcessNumber(){
		...
		Assert.assertTrue(new BigDecimal(betAmount).compareTo(new BigDecimal(21.0F)) == 0);
		Assert.assertTrue(betTotal == 21);
		maxWinAmountCompare = MathUtil.multiply(1, 
				times, 
				Float.class);
		maxWinAmountCompare = MathUtil.multiply(maxWinAmountCompare, 
