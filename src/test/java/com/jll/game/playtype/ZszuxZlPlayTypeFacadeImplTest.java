...
public class ZszuxZlPlayTypeFacadeImplTest extends ServiceJunitBase{
		
	public ZszuxZlPlayTypeFacadeImplTest(String name) {
		super(name);
	}	
	...
	final String facadeName = "zszux|中三组选/zlfs";
	...
	public void testPreProcessNumber(){
		...
		Assert.assertTrue(new BigDecimal(betAmount).compareTo(new BigDecimal(120.0F)) == 0);
		Assert.assertTrue(betTotal == 120);
		maxWinAmountCompare = MathUtil.multiply(1, 
				times, 
				Float.class);
		maxWinAmountCompare = MathUtil.multiply(maxWinAmountCompare, 
